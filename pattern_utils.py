...
def check_patterns_and_replace(question):
    ...
    """
	Checks the question (string) input by the user for one of the predefined patterns

	Args:
		question: String, represneting the inquiry of the user
			
	Returns:
		Returns:
			pat_matched: Boolean, indicating that pattern was matching to predefined patterns.
			new_sentence: String, represneting the statement to be said by the bot.
			para_list: List, containing the parameter(s) modified/affected as a result of matching a pattern.
...
def _check_arithmetic_pattern_and_replace(sentence):
    ...
    """
	Checks the if sentence (string) input by the user matches an arithmetic evaluation pattern

	Args:
		sentence: String, represneting the inquiry of the user
			
	Returns:
		Returns:
			A boolean, indicating that pattern matched an arithmetic evaluation pattern.
			sentence: String, original input to function in case of mismatch
			new_sentence: String, represneting the statement to be said by the bot.
			num_list: List, containing the parameter(s) modified/affected as a result of matching a pattern.
...
def _contains_arithmetic_pattern(sentence):
	...
	Returns:
		Returns:
			A boolean, indicating that pattern matched an arithmetic evaluation pattern.
			ind_list: List, containing the indicies of the numbers.
			num_list: List, containing the number(s) modified/affected as a result of matching a pattern.
...
def _text2int(text):
    ...
    """
	Converts text represneting numerical value into digits

	Args:
		text: String, represneting the number
			
	Returns:
		Returns:
			An integer which represents the numerical value of the input text
...
def _check_not_username_pattern_and_replace(sentence):
    ...
    """
	Checks the if sentence (string) input by the user matches a username pattern.
	...
	Returns:
		Returns:
			A boolean, indicating that pattern matched a username pattern
			sentence: String, original input to function in case of mismatch
			new_sentence: String, represneting the statement to be said by the bot.
			para_list: List, containing the parameter(s) modified/affected as a result of matching the pattern.
...
def _check_username_callme_pattern_and_replace(sentence):
    ...
    """
	Checks the if sentence (string) input by the user matches a username aliasing pattern.
	...
	Returns:
		Returns:
			A boolean, indicating that pattern matched a username aliasing pattern
			sentence: String, original input to function in case of mismatch
			new_sentence: String, represneting the statement to be said by the bot.
			para_list: List, containing the parameter(s) modified/affected as a result of matching the pattern.
    ...
    pat_call = re.compile(r'(\s|^)call\s+me\s+(.+?)(\s(please|pls|plz))?(\s\.|\s,|\s!|$)', re.IGNORECASE)
